...
fn default_response() -> MockResponse {
    MockResponse {
        status: 200,
        body: msg_envelope("default", vec![json!({"type": "text", "text": "no scenario matched"})], None),
    }
}

fn msg_envelope(id_suffix: &str, content: Vec<serde_json::Value>, usage: Option<serde_json::Value>) -> serde_json::Value {
    ...
    vec![
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("streaming_text", vec![
                    json!({"type": "text", "text": "Hello from mock stream."})
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("read_file_roundtrip", vec![
                    tool_use_block("tu-read-1", "Read", json!({"file_path": "/tmp/test.txt"}))
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("write_file_allowed", vec![
                    tool_use_block("tu-write-1", "Write", json!({"file_path": "/tmp/out.txt", "content": "hello"}))
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 403,
                body: msg_envelope("write_file_denied", vec![
        ...
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("grep_chunk_assembly", vec![
                    tool_use_block("tu-grep-1", "Grep", json!({"pattern": "TODO", "path": "/tmp/src"}))
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("multi_tool_turn", vec![
                    tool_use_block("tu-mt-1", "Read", json!({"file_path": "/tmp/a.txt"})),
                    tool_use_block("tu-mt-2", "Grep", json!({"pattern": "error", "path": "/tmp/b"})),
        ...
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("bash_stdout_roundtrip", vec![
                    tool_use_block("tu-bash-1", "Bash", json!({"command": "echo hello"}))
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("bash_permission_approved", vec![
                    json!({"type": "text", "text": "Bash execution approved."})
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 403,
                body: msg_envelope("bash_permission_denied", vec![
                    json!({"type": "text", "text": "Bash execution denied by policy."})
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("plugin_tool_roundtrip", vec![
                    tool_use_block("tu-plugin-1", "mcp__test__search", json!({"query": "test"}))
                ], None),
            },
        },
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("auto_compact_triggered", vec![
        ...
        MockScenario {
            ...
            response: MockResponse {
                status: 200,
                body: msg_envelope("token_cost_reporting", vec![
                    json!({"type": "text", "text": "Cost report generated."})
...
impl ParityTestRunner {
    ...
    pub fn run_scenario(&mut self, scenario_name: &str) -> Result<ParityResult, String> {
        let exists = self.service.scenarios.iter().any(|s| s.name == scenario_name);
        if !exists {
...
mod tests {
    ...
    fn unmatched_request_returns_default() {
        ...
        assert_eq!(
            svc.captured_requests()[0].matched_scenario,
            None
        );
    }
