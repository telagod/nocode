...
pub fn execute_team_create(call: ToolCallInput) -> ToolExecutionTrace {
    ...
    let summary = format!(
        "team '{team_name}' created with {count} subtasks: {}",
        subtasks.join(", ")
    );
...
pub fn execute_team_delete(call: ToolCallInput) -> ToolExecutionTrace {
    ...
    let progress = ToolProgressUpdate::new(
        call.tool_use_id.clone(),
        format!("deleting team {team_name}"),
    );
...
mod tests {
    ...
    fn team_create_parses_subtasks() {
        ...
        assert!(
            msg.contains("team 'alpha' created with 3 subtasks"),
            "{msg}"
        );
    }
    ...
    fn team_create_missing_name_fails() {
        ...
        assert!(
            trace
                .result
                .message()
                .contains("missing required argument: team_name"),
            "{}",
