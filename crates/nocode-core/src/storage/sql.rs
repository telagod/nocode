...
impl SqlStore {
    pub fn new(base_dir: &str) -> Result<Self, String> {
        ...
        Ok(Self { base_dir: base_dir.to_string() })
    }
    ...
    pub fn list_volumes(&self) -> Result<Vec<String>, String> {
        ...
        for entry in dir {
            ...
            if let Some(date) = name.strip_prefix("nocode_").and_then(|s| s.strip_suffix(".db")) {
...
pub fn global_sql_store(base_dir: &str) -> &'static Arc<Mutex<SqlStore>> {
    GLOBAL_SQL_STORE.get_or_init(|| {
        Arc::new(Mutex::new(SqlStore::new(base_dir).expect("sql store init")))
    })
}
...
mod tests {
    ...
    fn insert_and_get_messages() {
        ...
        store.insert_message("sess-msg", "user", "hello", 1, 10).unwrap();
        store.insert_message("sess-msg", "assistant", "hi there", 1, 20).unwrap();
